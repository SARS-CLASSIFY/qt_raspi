...
void WS2812_DMAInit(WS2812_Dev *dev, DMA_Stream_TypeDef *DMAStream, uint32_t DMARequest, IRQn_Type DMAIRQ)
{
  ...
  dev->DMAHandle.Init.Request = DMARequest;
...
{
  uint8_t i;
  if (WS2812_counter >= WS2812_currDev->len * 8 + 8) // boundary condition, modify this if necessary
  {
